...
namespace Kattis
{
    class Program
    {
        class Group
        {
            ...
            // assign this group its contents and return the rest of the input that does not belong to this group
            public string Assign(string input)
            {
                return "";
                // input is always starting with an {
                ...
                //else
                //{
                //    Regex groupContentsRegex = new Regex(@"{[^}]*}");
                //    Match match = groupContentsRegex.Match(input);

                //    if (!match.Success) throw new Exception("grop match failed");
        ...
        static void Main(string[] args)
        {
            ...
            //input = @"{{{},{},{{}}}}"; // score 16
            //input = @"{{<a!>},{<a!>},{<a!>},{<ab>}}"; // 3
            //input = "{<{o\"i!a,<{i<a>}"; // 10 removed
            //input = "{{},{}}"; // 0 removed

            // do 1 pass and remove all ! and the char they are negating
            ...
            {
                // two ifs to remove ! and the char after it
                if (removeNext)
                {
                    removeNext = false;
                    continue;
            ...
            // Remove all garbage groups
            Regex garbageRegex = new Regex(@"<[^>]*>");

            foreach (Match m in garbageRegex.Matches(input))
            {
                var derp = m.Value;
                removedChars += m.Value.Length - 2; // dont include the < >
            }

            var cleanedInput = garbageRegex.Replace(input, "");

            Console.WriteLine(cleanedInput);

            // now we only have {} things and , commas left.

            input = cleanedInput;

            //Group group = new Kattis.Program.Group();
            //group.Assign(input);
            ...
            for (int i = 0; i < input.Length; i++)
            {
                ...
                if (current == '}')
                {
                    ...
                    {
                        // if this group has a comma afterward we stay at the same level
                        // But since we did not we go down one level
                        level--;
                    }
